...
// TODO: change a different plugin that, on its own, adds the preview
// above syntax highlights instead of combining it with this
...
module.exports = (
  ...
  visit(markdownAST, 'inlineCode', node => {
    ...
    if (value.startsWith('embed:')) {
      ...
      const previewCode = fileContents;

      const code = fileContents
        .split(`\n`)
        .filter((line, index) => {
          const returnValue = filterDirectives(
            line,
      ...
      let preview;
      if (language === 'html') {
        preview = `<div class="gatsby-highlight-live">${previewCode}</div>`;
      }

      const snippet = `<div class="gatsby-highlight">
      ...
      node.value = `<div class="gatsby-code-preview">${preview +
        snippet}</div>`;
    }
  });
